...
ctx = execjs.compile(
    ...
    var gr = g(40, undefined, undefined)
    gr = g(gr.substring(0, 10), undefined, undefined)
    ...
    var cr = new Date
    var lr = cr.getTime()
    var ur = 6e4 * cr.getTimezoneOffset()
    var dr = lr + ur + 36e5 * 8
    var mr = new Date(dr)
    var pr = mr.getDate() + 9 + 9 ^ 10
    // 一天刷新一次
    var day = parseInt(pr)
    var gr = g(day, undefined, undefined)
    gr = g(gr.substring(0, 10), undefined, undefined)
    console.log(gr)

    var br = mr.getDay() + 11397
    function url() {
        var vr = "https://a2.m1907.top:404/api/v/?z=".concat(gr, "&jx=")
        return vr
    }
    """)
move_name = "西游记"
req_url = f"{ctx.call('url')}{move_name}&s1ig=11398&g="
header = {
    'accept-encoding': 'gzip, deflate, br',
    'accept-language': 'zh-CN,zh;q=0.9,en;q=0.8',
    'cache-control': 'no-cache',
    'dnt': '1',
    'origin': 'https://z2.m1907.top:404',
    'pragma': 'no-cache',
...
res = requests.get(url=req_url, headers=header).text
res = json.loads(res)
# print(res['data'])
for i in res['data']:
    name = i['name']
...
# def download(url):
#     yt_dlp.downloader -o "阿凡达2.mp4" "https://sod.bunediy.com/20221215/hlpxaybJ/index.m3u8" --external-downloader aria2c --external-downloader-args "-x 16 -k 1M"
